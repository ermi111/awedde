...
export class Scroll {
    ...
    static toggleClassOnScroll(element, className, offset) {
        window.addEventListener('scroll', () => {
            ...
            if (scrollPosition > offset) {
                ClassUtility.addClass(className);
            } else {
    ...
    static animateOpacity(element, startOpacity, endOpacity, duration, callback) {
        ...
        const animate = (timestamp) => {
            ...
            InlineStyle.addStyles(element, {opacity: opacity.toString()});
            if (progress < 1) {
    ...
    static animateHeight(element, startHeight, endHeight, duration, callback) {
        ...
        const animate = (timestamp) => {
            ...
            InlineStyle.addStyles(element, {height: height + 'px'});
            
            if (progress < 1) {
                requestAnimationFrame(animate);
            } else {
                InlineStyle.addStyles(element, {height: ''}); // Reset height to allow content to flow
                if (callback) {
                    callback();
                }
            }
    ...
    static disableScroll() {
        InlineStyle.addStyles(document.body, {overflow: 'hidden'});
    }
     ...
     */
    static enableScroll() {
        InlineStyle.addStyles(document.body, {overflow: ''});
    }
